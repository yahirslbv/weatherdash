    ...
    <div class="py-8 bg-[#0B132B] min-h-screen font-sans text-slate-200" x-data="{ tab: 'hora' }">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            @if($current)
                ...
                <div x-show="tab === 'hora'" x-transition:enter="transition ease-out duration-200" class="space-y-6">
                    <div class="bg-[#15203D] rounded-[24px] shadow-lg border border-[#1E2D56] p-8 overflow-hidden">
                            ...
                            <div class="relative w-full min-w-[700px] h-36 mt-4">
                                ...
                                @foreach($hourly as $index => $hour)
                                    ...
                                    <div class="absolute top-0 bottom-0 flex flex-col items-center group cursor-pointer" style="left: {{ $x }}%; transform: translateX(-50%); width: 40px;">
                                        <span class="absolute font-bold text-blue-300 text-[10px] opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap bg-[#0B132B] px-1.5 py-0.5 rounded border border-[#1E2D56] shadow-lg z-20 pointer-events-none" style="top: calc({{ $y }}% - 50px);">
                                            {{ $hour['prob'] }}% ☔
                                        </span>
                                        <span class="absolute font-extrabold text-white text-sm transition-transform group-hover:-translate-y-2 pointer-events-none" style="top: calc({{ $y }}% - 25px);">
                                            {{ $hour['temp'] }}°
                                        </span>
                ...
                <div x-show="tab === '7dias'" style="display: none;" x-transition:enter="transition ease-out duration-150">
                    <div class="bg-[#15203D] rounded-[24px] shadow-lg border border-[#1E2D56] p-6 sm:p-8">
                        ...
                        <div class="flex flex-col gap-3">
                            @foreach(array_slice($daily, 0, 7) as $day)
                                <div class="flex items-center justify-between bg-[#0B132B]/40 p-4 rounded-xl border border-[#1E2D56]/50 hover:bg-[#1E2D56]/30 transition">
                ...
                <div x-show="tab === '14dias'" style="display: none;" x-transition:enter="transition ease-out duration-150">
                    <div class="bg-[#15203D] rounded-[24px] shadow-lg border border-[#1E2D56] p-6 sm:p-8">
            ...
            @else
                <div class="bg-[#15203D] rounded-[24px] shadow-lg p-16 text-center border border-[#1E2D56]">
                    <p class="text-[#829AB1] font-medium text-lg">Cargando pronóstico...</p>
                </div>
            @endif
